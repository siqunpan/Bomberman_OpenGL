...
class TIMER
{
  ...
  private:
    ...
    float last_global_timer;   // en ms : dernier appel ?l'horloge (pour gérer les var. global_timer_***)
    ...
    float local_timer_30s;     //    |   temps, le booléen associ?passe ?true, et le compteur est RAZ.
  ...
  public:
    ...
    bool  global_timer_30s;			// La fonction update_horloge() doit être appelée ?chaque frame pour une
